...
for (const v of VIDEOS){
  try {
    const segs=[];
    ff(`-i "${INTRO}" -loop 1 -i "_v6_${v.id}_intro.png" -filter_complex "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1[v0];[1:v]fade=t=in:st=0:d=0.3:alpha=1[ov];[v0][ov]overlay=0:0[v]" -map "[v]" -t 4 -c:v libx264 -crf 20 -pix_fmt yuv420p -r 30 _s_${v.id}_intro.mp4`);
    segs.push(`_s_${v.id}_intro.mp4`);
    v.scenes.forEach((s,i)=>{
      ff(`-f lavfi -i "gradients=s=1080x1920:${BG[s.bg]||BG.pink}:d=${s.dur}:r=30" -ss ${s.t} -t ${s.dur} -i "${REC}" -loop 1 -i "_v6_${v.id}_s${i}.png" -filter_complex "[1]scale=${SW}:${SH}:force_original_aspect_ratio=increase,crop=${SW}:${SH}[app];[0][app]overlay=${SX}:${SY}[bg];[bg][2]overlay=0:0[v]" -map "[v]" -t ${s.dur} -c:v libx264 -crf 20 -pix_fmt yuv420p -r 30 _s_${v.id}_s${i}.mp4`);
